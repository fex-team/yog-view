...
var createHanlder = module.exports = function(fis, bigpipe, views) {
    ...
    bigpipe && bigpipe
        ...
        .on('before-pagelet-render', function(pagelet, locals) {
            var layer = locals._yog;
            var fork, subpagelets, origin;

            if (!layer)return;

            console.log('before-pagelet-render');
        ...
        .on('after-pagelet-render', function(pagelet, locals) {
            var layer = locals._yog;
            ...
            if (layer.getResourceMap()) {
                pagelet.scripts.push('require.resourceMap(' +
                    JSON.stringify(layer.getResourceMap()) + ');');
            }

            pagelet.styles.push.apply(pagelet.styles, styles.embed);
            pagelet.scripts.push.apply(pagelet.scripts, scripts.embed);

            styles.urls && pagelet.css.push.apply(pagelet.css, styles.urls);
            scripts.urls && pagelet.js.push.apply(pagelet.js, scripts.urls);
        });
